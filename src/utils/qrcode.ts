...
function appendrs(data: any, dlen: number, ecbuf: any, eclen: number) {
  // tslint:disable-next-line:one-variable-per-declaration
  let i, j, fb;
...
function genframe(instring: string[]) {
  // tslint:disable-next-line:one-variable-per-declaration
  let x, y, k, t, v, i, j, m;
...
export default {
   ...
   */
  // @ts-ignore
  draw(str, ctx, startX, startY, cavW, cavH, bg, color, ecc = ecclevel) {
    const that = this;
